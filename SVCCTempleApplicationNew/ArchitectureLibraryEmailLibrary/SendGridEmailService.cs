    ...
    public class SendGridEmailService
    {
        public void SendEmail(string emailDirectory, string aspNetUserId, KeyValuePair<string, string> fromEmailAddress, List<KeyValuePair<string, string>> replyToEmailAddresses, string emailSubject, string emailBody, List<KeyValuePair<string, string>> toEmailAddresses, string execUniqueId, string privateKey = "", List<KeyValuePair<string, string>> ccEmailAddresses = null, List<KeyValuePair<string, string>> bccEmailAddresses = null, List<string> emailAttachmentFileNames = null, bool saveEmail = true)
        {
            ...
            try
            {
                ...
                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00002000 :: Email Client, From & To Email", "FromEmailAddress", fromEmailAddress.Key, "FromEmailDisplay", fromEmailAddress.Value, "ToEmailAddress", toEmailAddresses[0].Key);
                ...
                foreach (var xyz in toEmailAddresses)
                {
                    toEmails.Add(new EmailAddress(xyz.Key));
                }
                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00003000 :: Email Message");
                var emailMessage = MailHelper.CreateSingleEmailToMultipleRecipients(fromEmail, toEmails, emailSubject, emailBody, emailBody, true);
                ...
                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00019000 :: SendGrid - After SendEmailAsync", "response.Result", response.Result.ToString());
